...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    return int(year), int(number) # makes the code pass succesfully (but still not correct
...
def record_invoice(invoice_file: TextIO,
    ...
    :param last_line_ptr The Position of the start of the last line in he file.
    ...
    if last_row:
        ...
        # if this line is empty, we'll start numbering from 1
        year = get_year()
...
with open(data_file, 'r+') as invoices:
    ...
    last_line = record_invoice(invoices, 'Squirrel Storage', 320.5, last_linep)
# # Test code:
